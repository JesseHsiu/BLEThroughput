...
//    }
    Label_NowStack.text = @"";
    Lable_TotalCostTime.text = @"";
    
    StartTime = [NSDate date];
    NSLog(@"!! Start to send 1000 datas, and start time: %@",StartTime);
...
-(void)ACKReceivedSendNextData
{
    NSDate *UI_CostTimeNow = [NSDate date];
    Label_NowStack.text = [NSString stringWithFormat:@"Now: %d", NowSendingStack];
    Lable_TotalCostTime.text = [NSString stringWithFormat:@"Cost:%f", [UI_CostTimeNow timeIntervalSinceDate:StartTime]];
    Lable_EachInterval.text = [NSString stringWithFormat:@"Interval Now:%f  %2.2f Hz", [UI_CostTimeNow timeIntervalSinceDate:Each_Interval],1/[UI_CostTimeNow timeIntervalSinceDate:Each_Interval]];
    if (NowSendingStack != 1000) {
        NowSendingStack++;
        Each_Interval = [NSDate date];
        [self SendData];
    }
    else
    {
        EndTime = [NSDate date];
        TotalCostTime = [EndTime timeIntervalSinceDate:StartTime];
        NSLog(@"Mission Completed time cost: %f\n *Start Time:%@\n *End Time:%@",TotalCostTime, StartTime, EndTime);
    }
...
{
    char c = 'a';
    NSData * data = [[NSData alloc]initWithBytes:(char *)&c length:2];//means 2 bytes only!!!
//     NSData * data = [MsgToArduino.text dataUsingEncoding:[NSString defaultCStringEncoding]];
    if(data.length > 20)
    {
        int i = 0;
